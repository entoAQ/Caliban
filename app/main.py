...
#
# Not a production pipeline stage. There is no threshold, no recorded
# decision, no automatic anything downstream of this yet -- just a way to
# run the current wording against a real reject photo and look at what it
# says, from the Dépistage visuel page's Rejets tab. Source of truth for the
# wording is this function; rig/prompt_reject_v1.txt carries the same text
# plus the status/design notes, kept in sync by hand until this is proven
# and worth making DB-sourced the way the MEO prompts are.
#
# Deliberately NOT in the vision_prompts table: that table's rows are read
# by parse_band_response, which expects a BANDE: MEO-percentage line. This
# prompt's answer shape (LARVES/MATIERE_ETRANGERE/DECISION/...) is not that,
# and a row in that table is offered to /azure-band-test -- inserting it
# there would put a reject-decision prompt one wrong click away from being
# run as if it were a MEO reading.
...
@app.get("/admin/reject-captures")
def admin_reject_captures(limit: int = 50, operator: dict = Depends(require_role("qc"))):
    """Recent reject-stream photos, for the Dépistage visuel page's Rejets
    tab. Lists capture_commands directly rather than a dedicated table --
    there isn't one yet, and the queue row already carries everything
    needed to browse and test against: id, the RJ- identity, and when it
    was taken."""
    resp = (
        supabase.table("capture_commands")
        .select("id, lot_number, requested_at, completed_at")
        .eq("kind", "capture_reject")
        .eq("status", "done")
...
@app.post("/admin/reject-vision-test")
def admin_reject_vision_test(command_id: str = Form(...), operator: dict = Depends(require_role("qc"))):
    """Runs the draft reject prompt against one already-captured reject
    photo and returns what it said. Test only -- nothing is recorded
    anywhere, same as a re-score.

    allowed_kinds is 'capture_reject' only, the mirror image of
    /azure-band-test's default: this endpoint must never be usable to
    analyse a real line sample under a different prompt, the same way
    /azure-band-test must never analyse a reject photo."""
