...
#
# Gated behind reject_vision_enabled() (default off) and run as a background
# task from capture_commands_complete, same shape as call_density_vision --
# see that function's comment and reject_vision_enabled's docstring for why.
#
...
def reject_vision_enabled():
    ...
    unproven (see reject_vision_prompt), and an earlier attempt to wire it up
    as a synchronous, browser-triggered call led to a deploy that took ~3x
    longer than normal to come up healthy, twice, for reasons never actually
    confirmed (no server log access at the time). This version calls it the
    same way call_density_vision is called -- a background task scheduled
    after the rig's HTTP response already went back, never in the request
    path of anything a person is waiting on -- but the flag stays off by
    default anyway, so the code can be deployed and proven stable before it
    ever actually executes. Flip it on in system_config once that's true, no
    redeploy needed.
    """
    try:
        resp = (
...
def _load_rig_capture(command_id, caller, is_operator, allowed_kinds=("capture",)):
    ...
    the image-preview endpoint below widens it, since an operator or QC
    reviewing a reject photo is harmless.
    """
    resp = (
...
def operator_capture_image(command_id: str, operator: dict = Depends(require_capture_role())):
    ...
    Also serves a 'capture_reject' photo -- an operator or QC reviewing a
    reject-stream frame needs to see it too, even though it can never reach
    /azure-band-test (see _load_rig_capture).
    """
    is_operator = operator.get("role") == "operator"
...
def admin_reject_captures(limit: int = 50, operator: dict = Depends(require_role("qc"))):
    ...
    tab -- browsing only, no analysis. Lists capture_commands directly
    rather than a dedicated table -- there isn't one yet, and the queue row
    already carries everything needed to browse: id, the RJ- identity, and
    when it was taken."""
    resp = (
        supabase.table("capture_commands")
