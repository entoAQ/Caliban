...
_me_pct_test_id = "__unset__"  # sentinel distinct from None, so a genuine "not found" isn't re-queried every request


def get_me_pct_test_id():
    """Looks up test_definitions.id for code='me_pct' once, caching the
    result -- this ID never changes, no reason to query it on every
    single band-test call."""
    global _me_pct_test_id
    if _me_pct_test_id == "__unset__":
        resp = supabase.table("test_definitions").select("id").eq("code", "me_pct").limit(1).execute()
        _me_pct_test_id = resp.data[0]["id"] if resp.data else None
    return _me_pct_test_id
...
async def azure_band_test(
    ...
    try:
        ...
        if lot_id:
            # Real lot matched -- pull the authoritative lab value
            ...
            #
            # Two plain .eq() lookups rather than a PostgREST embedded-
            # resource filter -- that syntax works in principle but was
            # never tested against a real connection from this
            # environment, and getting a silent, wrong match here would
            # be a genuinely bad way to fail.
            me_test_id = get_me_pct_test_id()
            if me_test_id:
                me_resp = (
                    supabase.table("test_results")
                    .select("result_value")
                    .eq("lot_id", lot_id)
                    .eq("test_id", me_test_id)
                    .eq("is_superseded", False)
                    .limit(1)
                    .execute()
                )
                if me_resp.data:
                    real_pct_value = me_resp.data[0]["result_value"]
