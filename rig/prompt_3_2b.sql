-- Prompt 3.2b -- 3.2c (3.2 with DENSITE removed) with ONLY the low-end
-- paragraph changed.
--
-- Everything else is 3.2c word for word, generated from the 3.2c text in
-- app/main.py rather than retyped, so any difference in results against
-- 3.2c is that paragraph and nothing else. Band labels and scale are
-- unchanged (coarse: <3%, 3-8%, 8-13%, >13%), so nothing operations sees
-- changes.
--
-- Updated 2026-09-19 to drop DENSITE (previously anchored the same way as
-- 3.2). It correlated with measured density at r = 0.19 -- see
-- prompt_3_3_draft.sql -- weak enough that every variant since 3.3 dropped
-- it, but this one predated that finding and system_config still had it
-- live as operator_settings.low_variant, asking a question already known
-- not to work. Density estimation now has its own instrument (the bench
-- rig's time-of-flight sensor), fully decoupled from this prompt, so there
-- is nothing left for DENSITE to do here. Editing in place, not a new
-- label: this is the db-sourced side of the prompt family, where that is
-- the established practice (prompt_hash and vision_prompt_history already
-- keep earlier estimates interpretable against the text that produced
-- them) -- unlike the code-sourced 3.0/3.1/3.2/3.2c lineage, which never
-- rewrites in place because it has no such history table.
--
-- Original rationale, unchanged: truly low samples read 3-8%. 13 of 13 real
-- values under 3% read high in the 2026-09 data, and a 2.54% tray on
-- 2026-09-11 read 3-8% on every rotation. 3.2 describes <3% as "a few small
-- isolated specks", which makes a real 2-3% tray -- where MEO is plainly
-- visible -- look like 3-8%. 3.2b describes what a 2-3% tray actually looks
-- like, and restores the explicit "NOT 3-8%" that 3.0 had and 3.1 dropped.
--
-- Applies to QC and operator captures alike, and each estimate records the
-- prompt that produced it.
--
-- Check first that 3.2c is not overridden in the database (if this returns
-- a row, its text is what is live, and this file was built from a stale
-- code copy):
--   select label, updated_at from vision_prompts where label = '3.2c';
...
JUSTIFICATION: [one sentence, in French, what drove this choice]$prompt$,
        'coarse',
        '3.2c (3.2 minus DENSITE) with only the <3% paragraph rewritten: describes a real 2-3% tray, restores "NOT 3-8%". No DENSITE.')
